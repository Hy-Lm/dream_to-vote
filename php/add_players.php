	        ...
	        if(isset($_POST['sub'])){
	        	...
	        	// echo $name.$file;
	        	    if(is_uploaded_file($file1['tmp_name'])){
	        	        //路径
						$hou=pathinfo($file1['tmp_name'],PATHINFO_EXTENSION);
						$video=rand(10,1000).time().'.'.$hou;//在图片名称后加入时间戳，避免重名文件覆盖
						move_uploaded_file($file1['tmp_name'], "images/".$video);
	        	}
				//音频
				...
				// echo $name.$file;
				    if(is_uploaded_file($file2['tmp_name'])){
				        //路径
						$hou=pathinfo($file2['tmp_name'],PATHINFO_EXTENSION);
						$voice=rand(10,1000).time().'.'.$hou;//在图片名称后加入时间戳，避免重名文件覆盖
						move_uploaded_file($file2['tmp_name'], "images/".$voice);
				}
				//多图
				//取文件信息、
					$arr=$_FILES['imgs'];
